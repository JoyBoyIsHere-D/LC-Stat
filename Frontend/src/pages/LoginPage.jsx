...
import { getUserByUid } from '../config/userService'
...
const LoginPage = () => {
	...
	const handleSubmit = async e => {
		...
		try {
			...
			// If no profile, redirect to complete profile (this shouldn't normally happen)
			if (!userProfile) {
	...
	const handleGoogleSignIn = async () => {
		...
		try {
			...
			if (result._tokenResponse?.isNewUser) {
				...
				await signOut(auth) // Sign out the user since this is login, not signup
				setLoading(false)
				return
			}
			...
			<div className="w-full max-w-md">
				<div className="bg-white dark:bg-gray-800 rounded-xl shadow-md p-8">
					<div className="text-center mb-8">
						...
						<p className="text-gray-600 dark:text-gray-400 mt-2">Sign in to your LC-Stat account</p>
					</div>
					...
					<form onSubmit={handleSubmit} className="space-y-6">
						...
						<button
							type="submit"
							disabled={loading}
							className={`w-full py-3 px-4 rounded-md font-medium text-white bg-gradient-to-r from-blue-500 to-purple-600 hover:from-blue-600 hover:to-purple-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500 transition-all ${loading ? 'opacity-70 cursor-not-allowed' : ''
								}`}
						>
							{loading ? (
